...
from semantic_ner_search_demo.config.env import (
    ELSER_MODEL_ID,
    ES_API_KEY,
    ES_CLOUD_ID,
    ES_SEARCH_INDEX,
...
def main():
    ...
    selected_index = st.sidebar.selectbox(
        "Elasticsearch Index", [ES_SEARCH_INDEX], key="selected_index"
    )
    ...
    if st.button("Search"):
        st.session_state["search_results"] = fetch_data(
            selected_index, semantic_query, regular_query, start_date, end_date
        )
        st.session_state["selected_tags"] = {
            tag_type: set() for tag_type in ["LOC", "PER", "MISC"]
        }  # Reset filters on new search

    for tag_type in ["LOC", "PER", "MISC"]:
        current_tags = get_unique_tags(tag_type, st.session_state["search_results"])
        st.session_state["selected_tags"][tag_type] = st.sidebar.multiselect(
            f"Filter by {tag_type}", current_tags, key=f"filter_{tag_type}"
        )

    filtered_results = filter_results_by_tags(
        st.session_state["search_results"], st.session_state["selected_tags"]
    )
...
def fetch_recent_data(index_name, size=100):
    try:
        query_body = {
            "size": size,
            "sort": [
                {"@timestamp": {"order": "desc"}},  # Primary sort by date
            ],
        }
...
def get_date_range(index_name):
    max_date_aggregation = {"max_date": {"max": {"field": "@timestamp"}}}

    min_date_aggregation = {"min_date": {"min": {"field": "@timestamp"}}}
    ...
    if max_date:
        ...
        max_date = datetime_parse(max_date)
    else:
        max_date = datetime.today().date()

    if min_date:
        # min_date = datetime.strptime(min_date, "%a, %d %b %Y %H:%M:%S GMT")
        min_date = datetime_parse(min_date)
    else:
...
def update_results(results):
    try:
        for result_item in results:
            ...
            # Using datetime for simple ISO formats (without timezone or with 'Z' for UTC)
            try:
                dt = datetime.fromisoformat(timestamp.replace("Z", "+00:00"))
            except ValueError:
                # Using dateutil for more complex formats, including timezones
                dt = datetime_parse(timestamp)

            # Now, format it into a more human-friendly format
            timestamp = dt.strftime("%B %d, %Y at %H:%M %p")
...
def fetch_data(
    index_name, semantic_query, regular_query, start_date=None, end_date=None, size=100
):
    try:
        ...
        if regular_query:
            query_body["query"]["bool"]["should"].append(
                {"query_string": {"query": regular_query, "boost": 8}}
            )

        # Add date range if provided
        if start_date or end_date:
            date_range_query = {"range": {"@timestamp": {}}}

            if start_date:
                # date_range_query["range"]["@timestamp"]["gte"] = start_date.strftime(
                #     "%a, %d %b %Y %H:%M:%S GMT"
                # )
                date_range_query["range"]["@timestamp"]["gte"] = start_date.isoformat()

            if end_date:
                date_range_query["range"]["@timestamp"]["lte"] = end_date.isoformat()
                # date_range_query["range"]["@timestamp"]["lte"] = end_date.strftime(
                #     "%a, %d %b %Y %H:%M:%S GMT"
                # )
